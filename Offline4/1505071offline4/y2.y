...
start : program {
		           ...
		           // cout<<$1->code;
			        ...
			        for(int i = 0; i<variables.size() ; i++){
				        asm_file  << variables[i] << " dw ?\n";			
			        }

			        for(int i = 0 ; i< arrays.size() ; i++){
				        asm_file  << arrays[i] << " dw " << arraySizes[i] << " dup(?)\n";
			        }
...
variable : 	ID
			{
				...
				if(temp == NULL){
					...
					variables.push_back($$->Name+to_string(table->count));
					$$->Type="notarray";
				}	
	 		...
	 		| ID LTHIRD expression RTHIRD
			{
				...
				if(temp == NULL){
					...
					$$->Type="array";
					$$->Name=$$->Name+to_string(table->count);
					arrays.push_back($$->Name+to_string(table->count));
