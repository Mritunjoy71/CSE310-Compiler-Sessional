...
func_declaration : 	type_specifier ID LPAREN parameter_list RPAREN SEMICOLON
					{
						...
						if(temp != NULL){
							errorFile << "Error at line " << line_no << " Function "<< $2->getName() <<" already declared" << endl << endl;
							semErrors++;
						}else {
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN compound_statement
				...
				SymbolInfo *temp = table->lookUp($2->getName(), "FUNC");
				if(args.size() != IDargs){
					errorFile << "Error at line " << line_no << " Parameter mismatch for Function "<< $2->getName() << endl << endl;
					args.clear(); IDargs = 0;
					semErrors++;
				}												
				if(temp != NULL){
					...
					if(temp->isFuncDefined()== true){
						errorFile << "Error at line " << line_no << "Function "<< $2->getName() <<" already defined" << endl << endl;
						semErrors++;
						args.clear();
						IDargs = 0;
					}
					else if(temp->getFuncRet() != $1->getVarType()){
						errorFile << "Error at line " << line_no << "Function "<< $2->getName() <<" :return type doesn't match declaration" << endl << endl;
						semErrors++;
						args.clear();IDargs = 0; 
					} 
					else if(temp->ParamList.size() != args.size()){
						errorFile << "Error at line " << line_no << "Function "<< $2->getName() <<" :Parameter list doesn not match declaration" << endl << endl;
						args.clear();IDargs = 0;
						semErrors++;					
					}
					else{
						for(int i = 0; i<temp->ParamList.size(); i++){
							if(temp->ParamList[i] != args[i]){
								errorFile << "Error at line " << line_no << "Function "<< $2->getName()<< " :argument mismatch" << endl << endl;
								args.clear();IDargs = 0;
								semErrors++;	
							}
						}				
...
compound_statement : 	LCURL 
						...
						} statements {table->printAll(logFile);}RCURL{table->exitScope();}
						{
 		    			...
 		    			| LCURL RCURL{logFile << "Line " << line_no << " : compound_statement : LCURL RCURL\n"<< endl;}
 		    			;
 		    
var_declaration : type_specifier declaration_list SEMICOLON{logFile << "Line " << line_no << " : var_declaration : type_specifier declaration_list SEMICOLON\n"<< endl;}
...
declaration_list : 	declaration_list COMMA ID
					{
						...
						if(variable_type == "VOID"){
							...
							if(temp != NULL){
							errorFile << "Error at line " << line_no << ": Variable "<< $3->getName() <<" already declared" << endl << endl;	
								semErrors++;	
							}
							else{
 					...
 					| declaration_list COMMA ID LTHIRD CONST_INT RTHIRD
					{
						...
						if(variable_type == "VOID"){
							...
							if(temp!= NULL){
							errorFile << "Error at line " << line_no << " : Array "<< $3->getName()<< " already declared" << endl << endl;
								semErrors++;			
							}
							else{
								SymbolInfo* temp2 = new SymbolInfo($3->getName(), $3->getType());
 					...
 					| ID
					{
						...
						if(variable_type == "VOID"){
							...
							if(temp!= NULL){
							errorFile << "Error at line " << line_no << ": Variable "<< $1->getName() <<" already declared" << endl << endl;	
								semErrors++;		
							}
							else{
					...
					| ID LTHIRD CONST_INT RTHIRD
					{
						...
						if(variable_type == "VOID"){
							...
							SymbolInfo* temp = table->lookUp($1->getName(), "ARA");
							if(temp!= NULL){
								errorFile << "Error at line " << line_no << ": Array "<< $1->getName() <<" already declared" << endl << endl;	
								semErrors++;
							}
							else{
								SymbolInfo* temp2 = new SymbolInfo($1->getName(), $1->getType());
...
statements : statement{logFile << "Line " << line_no << " : statements : statement\n"<< endl;}
	   | statements statement{logFile << "Line " << line_no << " : statements : statements statement\n"<< endl;}
	   ;
	   
statement : var_declaration{logFile << "Line " << line_no << " : statement : var_declaration\n"<< endl;}
	  | expression_statement{logFile << "Line " << line_no << " : statement : expression_statement\n"<< endl;}
	  | compound_statement{logFile << "Line " << line_no << " : statement : compound_statement\n"<< endl;}
	  ...
	  | WHILE LPAREN expression RPAREN statement{logFile << "Line " << line_no << " : statement : WHILE LPAREN expression RPAREN statement\n"<< endl;}
	  | PRINTLN LPAREN ID RPAREN SEMICOLON{logFile << "Line " << line_no << " : statement : PRINTLN LPAREN ID RPAREN SEMICOLON\n"<< endl;}
	  | PRINTLN LPAREN ID RPAREN error
	{
		errorFile << "Error at line " << line_no << "; missing" << endl << endl;
		semErrors++;
	} 		 		
	  | RETURN expression SEMICOLON{logFile << "Line " << line_no << " : statement : RETURN expression SEMICOLON\n"<< endl;}
...
expression_statement 	: SEMICOLON{logFile << "Line " << line_no << " : expression_statement : SEMICOLON\n"<< endl;}			
						| expression SEMICOLON {logFile << "Line " << line_no << " : expression_statement : expression SEMICOLON\n"<< endl;}
						|expression error
						{
							errorFile << "Error at line " << line_no << "; missing" << endl << endl;
							semErrors++;
...
variable : 	ID
			{
				...
				if(temp == NULL){
					...
					errorFile << "Error at line " << line_no << " : " << $1->getName() << " doesn't exist" <<  endl << endl;					
					semErrors++;
				}
				else{
					$$ = temp;
	 		...
	 		| ID LTHIRD expression RTHIRD
			{
				...
				errorFile << "Error at line " << line_no << " : " <<$1->getName() << " array index out of bounds" <<  endl << endl;				
						semErrors++;
						//temp->setAraIndex(0);
					} 
...
factor	: variable 
	...
	| ID LPAREN argument_list RPAREN
	{
	    ...
	    logFile << $1->getName() << endl << endl;
			...
			if(temp == NULL){
				errorFile << "Error at line " << line_no <<" : Function " <<$1->getName() <<" doesn't exist"<<endl << endl;
			}
			else{
				if(temp->getFuncRet() == "VOID"){
					errorFile << "Error at line " << line_no <<" : Function " <<$1->getName() <<" returns void"<<endl << endl;
				} 
				else{ 
