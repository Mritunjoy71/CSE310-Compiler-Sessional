...
public:
    ...
    ScopeTable(int m ,int scop_n,ScopeTable *st_cur)
    {
        bucket_size=m;
        this->st_id=scop_n;
...
public:
    ...
    SymbolTable(int m){
        count=1;
        curScope=NULL;
        ScopeTable *st  =new ScopeTable(m,1,curScope);
        st->parentScope=NULL;
        this->curScope=st;
       // present=curScope;
    ...
    {   
        count++;
        ScopeTable *st  =new ScopeTable(curScope->bucket_size,count,curScope);
        st->parentScope=curScope;
        this->curScope=st;
       // present=curScope;
