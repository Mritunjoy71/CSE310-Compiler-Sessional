...
func_declaration : 	type_specifier ID LPAREN parameter_list RPAREN SEMICOLON
					...
					{
						error_file << "Error at line " << line_no << "; missing" << endl << endl;
						errors++;
					}
		 			;
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN compound_statement
				...
				if(temp != NULL){
					//log_file << "Function " << $2->Name <<" already declared" << endl;
					...
					//log_file << $2->Name << " Function inserted" << endl;
					temp->IDType="FUNC";
...
parameter_list  : parameter_list COMMA type_specifier ID
				{
					...
					//log_file << $4->Name << endl << endl;
					args.push_back(variable_type);//changed from $3->VarType
					IDargs++;
					//SymbolInfo* temp = funcParam.InsertandGet($4->Name, $4->Type);					
					//temp->IDType="VAR";
					//temp->setVarType($3->VarType);
					$4->IDType="VAR";
					$4->setVarType(variable_type);//changed from $3->VarType
 				...
 				| type_specifier ID
				{
					...
					//log_file << $2->Name << endl << endl;
					//args.clear();
					args.push_back(variable_type);//$1->VarType
					IDargs++;
					//SymbolInfo* temp = funcParam.InsertandGet($2->Name, $2->Type);
					//temp->IDType="VAR";
					//temp->setVarType($1->VarType);
					$2->IDType="VAR";
					$2->setVarType(variable_type);//$1->VarType
					params.push_back($2);
...
declaration_list : 	declaration_list COMMA ID
					{
						...
						if(variable_type == "VOID"){
							...
							error_file << "Error at line " << line_no << ": Multiple Declaration of  "<< $3->Name << endl << endl;	
								...
								//SymbolInfo* temp2 = table.InsertandGet($3->Name, $3->Type);
 					...
 					| declaration_list COMMA ID LTHIRD CONST_INT RTHIRD
					{
						...
						//log_file << $3->Name<<"[" <<$5->Name<<"]"<<endl<<endl;
						if(variable_type == "VOID"){
 					...
 					| ID
					{
						...
						//log_file << $1->Name << endl << endl;
						if(variable_type == "VOID"){
							error_file << "Error at line " << line_no << " :variable type can't be void" << endl << endl;
							errors++;
						}
						else{
							SymbolInfo* temp = table->curScope->lookUp($1->Name);     //edit
							if(temp!= NULL){
					...
					| ID LTHIRD CONST_INT RTHIRD
					{
						...
						//log_file << $1->Name<<"[" <<$3->Name<<"]"<<endl<<endl;
						if(variable_type == "VOID"){
							error_file << "Error at line " << line_no << " :array type can't be void" << endl << endl;
							errors++;
						}
...
variable : 	ID
			{
				...
				if(temp == NULL){
					error_file << "Error at line " << line_no << " : " << $1->Name << " doesn't exist" <<  endl << endl;					
					errors++;
				}
				else{
					$$ = temp;
	 		...
	 		| ID LTHIRD expression RTHIRD
			{
				...
				error_file << "Error at line " << line_no << " : " <<$1->Name << " array index out of bounds" <<  endl << endl;				
					...
					if(temp->VarType == "INT"){
						while(temp->ints.size() <= temp->araIndex){
							temp->ints.push_back(0);
						}
					}
					else if(temp->VarType == "FLOAT"){
						while(temp->floats.size() <= temp->araIndex){
							temp->floats.push_back(0);
						}
					}
					else if(temp->VarType == "CHAR"){
						while(temp->chars.size() <= temp->araIndex){
							temp->chars.push_back('\0');
						}
					}
					$$ = temp;
					//variable_type = temp->VarType;
				}
				...
				if(vType == "INT"){
					if($1->IDType == "VAR"){
						...
						if($3->VarType == "INT"){
							...
							error_file << "Warning at line " << line_no <<" : Assigning float value to integer"<<endl << endl;
...
logic_expression 	: rel_expression
		 			...
		 			| rel_expression LOGICOP rel_expression
					{
						...
						if($1->VarType == "CHAR" || $3->VarType == "CHAR"){
							//log_file << "Logical operation not allowed for char datatype" << endl;
							temp->ints.push_back(0);
						}
...
rel_expression	: simple_expression 
				...
				| simple_expression RELOP simple_expression
				{
					...
					if(relop == "=="){
						if(type1 != type2){
							//log_file << "Type mismatch for == operand" << endl;						
						}
						else if(type1 == "INT"){
...
simple_expression 	: term
		 			...
		 			| simple_expression ADDOP term
					{
						...
						string addop = $2->Name;
						//log_file << $1->ints[0] << "+" << $3->ints[0] << endl;
						if(addop == "+"){
							if($1->IDType == "VAR"){
								if($3->IDType == "VAR"){						
									if($1->VarType == "FLOAT"){
	    ...
	    log_file << "Line " << line_no << " factor	: ID LPAREN argument_list RPAREN:\n"<< endl;
			...
			if(temp == NULL){
				...
				if(temp->FuncRet == "VOID"){
					...
					if(temp2->VarType == "INT")temp2->ints[0] = 0;
					else if(temp2->VarType == "FLOAT")temp2->floats[0] = 0;
					else if(temp2->VarType == "CHAR")temp2->chars[0] = '\0';
					$$ = temp2;
				}
			}
